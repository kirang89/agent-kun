...
export default function prExtension(pi: ExtensionAPI) {
	pi.registerCommand("pr", {
		description: "Commit relevant changes, create or update a PR (/pr against <branch> ready)",
...
Create or update a PR for this session.

Before creating or updating it:
- Use the git commit workflow for any required commit.
- Inspect git status and diffs.
- Check whether the current branch already has an open PR (for example, gh pr status/view).
...
- Use one or more concise Conventional Commit messages, splitting commits when changes are logically separate.
- Use ${args.targetBranch} as the PR base branch.

If an open PR already exists:
- Push any new commits to the existing PR branch.
- Review the full diff against the base branch and the existing PR title/body.
- Update the PR title and/or description only when the new commits make the current title/body incomplete, stale, or misleading.
...
- Write the PR title and description in ELI15 style (Explain Like I'm 15) — plain language any developer can grasp in seconds, no jargon or fluff. Imagine the reviewer skim-reads after lunch: lead with the user-facing problem, then what changed, in simple direct terms.
